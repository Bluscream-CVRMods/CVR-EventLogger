...
public static class Patches {
    public static void Patch(HarmonyLib.Harmony harmonyInstance) {
        _ = harmonyInstance.Patch(typeof(CVRVideoPlayer).GetMethod("SetVideoUrl"), prefix: new HarmonyMethod(typeof(Patches).GetMethod("OnSetVideoUrl", BindingFlags.Static | BindingFlags.NonPublic)));
    ...
    private static bool OnSetVideoUrl(string url, bool broadcast = true, string objPath = "", string username = null, bool isPaused = false) {
        if ((bool)Main.LogVideoPlayerSetting.BoxedValue) {
    ...
    public static void SetJoinTarget(string instanceId, string worldId) {
        if ((bool)Main.LogWorldsSetting.BoxedValue) {
...
public class Main : MelonMod {
    ...
    private void GameNetwork_Disconnected(object sender, DarkRift.Client.DisconnectedEventArgs e) {
        OnDisconnected("GameNetwork", sender, e);
    }

    private void CallsNetwork_Disconnected(object sender, DarkRift.Client.DisconnectedEventArgs e) {
        OnDisconnected("CallsNetwork", sender, e);
    }

    private void Api_Disconnected(object sender, DarkRift.Client.DisconnectedEventArgs e) {
        OnDisconnected("API", sender, e);
    }
